...
export async function startDisplay(root, canvas) {
  ...
  window.addEventListener("keydown", (ev) => {
    ...
    unlockAudio();
    if (ev.code === "Space") { mouse.space = true; ev.preventDefault(); }
    ...
    try {
      ...
      host = await createHost({
        ...
        handlers: {
          ...
          onInput(cid, v) { if (players.has(cid)) sim.setInput(cid, v); }
        }
      });
  ...
  function frame(now) {
    ...
    if (hudClock >= 0.25) { hudClock = 0; pushPads(); renderPlayers(); }
