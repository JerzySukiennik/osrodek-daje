...
export function buildSelectScene(levels, progress = {}) {
  ...
  const items = [
    ...
    { type: "bench", x: -4.4, z: 3.6, rot: 0 }, { type: "bench", x: 4.4, z: 3.6, rot: 0 }
  ];
  levels.forEach((lvl, i) => {
    const x = (i - (n - 1) / 2) * 5.6;
    items.push({ type: "barrel", x, z: -1.2, tag: "select:" + lvl.id });
    items.push({ type: "parasol", x, z: -3.4 });
  });
  return {
    id: "select", kind: "select", variant: "-", title: "Wybór levelu",
    blurb: "One barrel per diorama. Swallow it to play; the sign at the front takes you back.",
    arena: { hx: 11, hz: 7 }, spawns: [[0, 5.8], [-2.6, 5.8], [2.6, 5.8], [0, 4.8]], colors: GRASS,
    decals: [path([[-11, 1.2], [11, 1.2], [11, -0.4], [-11, -0.4]], "#f6c453"), blob(0, -2.4, 6, "#d3e655", 77)],
    items,
    levels,
    progress
  };
...
export function selectLabels(scene, starsPerLevel) {
  const out = [];
  for (const item of scene.items) {
    if (!item.tag) continue;
    const key = item.tag.split(":")[1];
    const pos = { x: item.x, z: item.z, y: (PROPS[item.type] ? PROPS[item.type].height : 1) + 0.45 };
    if (key === "back") {
      out.push({ tag: item.tag, ...pos, text: "BACK", sub: "to the menu" });
      continue;
    }
    const lvl = scene.levels.find((l) => l.id === key);
    const got = (scene.progress[key] || []).length;
    out.push({ tag: item.tag, ...pos, text: lvl.title, sub: `${got} / ${starsPerLevel} ★`, tone: got ? "owned" : "go" });
  }
  return out;
}
