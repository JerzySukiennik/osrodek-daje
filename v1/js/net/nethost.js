...
import {
  claimRoom, watchPeerSignals, sendAnswer, pushIce, clearPeerSignals, clearTaken
} from "./signaling.js";
...
export async function createHost({ roomCode, handlers } = {}) {
  ...
  function routeInput(cid, msg) {
    ...
    onInput(cid, { x: Number(msg.x) || 0, y: Number(msg.y) || 0, b: msg.b ? 1 : 0, q: q == null ? 0 : q });
  }
  ...
  function sendPad(cid, pad) {
    ...
    const msg = { ...pad, type: MSG.PAD };
    const key = JSON.stringify(msg);
    const ctxChanged = key !== st.lastCtx;
    st.lastCtx = key;
