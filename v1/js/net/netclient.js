...
export async function connectClient({ roomCode, cid, handlers } = {}) {
  ...
  fallbackTimer = setTimeout(() => toRelay("timeout"), RTC_FALLBACK_S * 1000);
  ...
  return {
    ...
    sendInput(v) {
      ...
      const msg = { type: MSG.INPUT, x: v.x, y: v.y, b: v.b ? 1 : 0, q: seq };
      seq = (seq + 1) % SEQ_MOD;
      if (mode === "rtc" && peer) return peer.sendInput(msg);
      if (mode === "relay" && relayed) return relayed.sendInput({ x: msg.x, y: msg.y, b: msg.b, q: msg.q });
      return false;
    },
    mode() { return mode; },
    close() {
